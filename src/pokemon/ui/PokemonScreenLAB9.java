...
public class PokemonScreenLAB9 extends Application {
	...
	public void start(Stage stage) throws Exception {
		...
		pkcImage = new Image(pikachu);
		
		avatar = new ImageView(avatarImage);
		avatar.setFitHeight(STEP_SIZE);
		avatar.setFitWidth(STEP_SIZE);
		avatar.setPreserveRatio(true);
		int randomx = new Random().nextInt(W/STEP_SIZE) * STEP_SIZE;
		int randomy = new Random().nextInt(H/STEP_SIZE) * STEP_SIZE;
		currentPosx = randomx; // this should be a random position
		currentPosy = randomy; // this should be a random position
		
		avatar.relocate(currentPosx, currentPosy);
		
		pkc = new ImageView(pkcImage);
		pkc.setFitHeight(STEP_SIZE);
		pkc.setFitWidth(STEP_SIZE);
		pkc.setPreserveRatio(true);
		
		System.out.println(currentPosx);
		Group mapGroup = new Group();
		
		mapGroup.getChildren().add(avatar);
		mapGroup.getChildren().add(pkc);
		pkc.setVisible(false);
		pkc.relocate(currentPosx, currentPosy);
